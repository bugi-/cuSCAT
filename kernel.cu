...
#include <stdio.h>
#include <stdlib.h>
// Cudaan ja valittuun satunnaislukugeneraattoriin liittyvi‰ asioita
#include <cuda.h>
...
// Kutsumismakroja helpompaan virheiden tarkistukseen
#define CURAND_CALL(x) if((x) != CURAND_STATUS_SUCCESS) { \
    printf("Error at %s:%d\n",__FILE__,__LINE__); \
    return EXIT_FAILURE;}
#define CUDA_CALL(x) {cudaError_t code = (x); \
	if((code) != cudaSuccess) { \
    printf("Error at %s:%d\n",__FILE__,__LINE__); \
...
__constant__ cloud_state *state; // Laitetaan kyseinen ominaisuus-struct nopealukuiseen vakiomuistiin


// Testailuun
__device__ void print_ray(ray *r) {
	printf("Intensity: %f\n", r->intensity);
...
__device__ ray* init_ray_planar(int *dims, curandStateMtgp32 *randStates) {
...
// Alustaa fotonipaketin x,y-tasolla
__device__ ray* init_ray_planar2(int *dims, curandStateMtgp32 *randStates) {
...
__device__ int simulate_ray(float *cloud, ray *cur_ray, curandStateMtgp32 *randStates) {
...
__global__ void trace_ray(float *cloud, float *map, int *dims, curandStateMtgp32 *randStates) {
...
__host__ float* generate_constant_density_cloud(int size) {
...
int main(void) {
	...
	// Cudan parametreja
	long randSeed = SEED;
	int minThreads = num_rays; // V‰hint‰‰n n‰in monta s‰iett‰
	int blockSize = 64; // MTGP32-satunnaislukugeneraattori tukee korkeintaan 256-kokoisia lohkoja.
	int numThreads = (minThreads / blockSize + 1) * blockSize; // Lopullinen s‰ikeiden m‰‰r‰ on minThreads tai sit‰ seuraava blockSize:n monikerta.
	int gridSize = numThreads / blockSize;

	// T‰h‰n listaan eri dimensioiden koot
	int *hdims = (int *)malloc(NUM_DIMS * sizeof(int));
	int *ddims;
	CUDA_CALL(cudaMalloc((void **)&ddims, NUM_DIMS * sizeof(int)));
	for (int i = 0; i < NUM_DIMS; i++) {
		hdims[i] = dim_size;
	}
	CUDA_CALL(cudaMemcpy(ddims, hdims, NUM_DIMS * sizeof(int), cudaMemcpyHostToDevice));
	...
	CURAND_CALL(curandMakeMTGP32Constants(mtgp32dc_params_fast_11213, dRandParams)); // K‰ytet‰‰n valmista parametrilistaa (ensimm‰inen argumentti)
	CURAND_CALL(curandMakeMTGP32KernelState(dRandStates, mtgp32dc_params_fast_11213, dRandParams, gridSize, randSeed)); // Tehd‰‰n jokaiselle lohkolle oma tila
